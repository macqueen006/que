...
if(isset($_GET['search'])){
    ...
    ?>
    <h2 style="background-color: gold;color: white;opacity: 60%;" class="text-center">Results For <u><?php echo $Search; ?></u></h2>
        ...
        <div class="container">
            <div class="row">
                    <div class="col-md-10 offset-md-1 col-lg-9 offset-lg-0">
                        ...
                        $query = "SELECT * FROM forum WHERE status='published' AND tags LIKE '%$Search%' ORDER BY id DESC ";
                        $result = Query($query);
                        confirm($result);
                        ...
                        $sql = "SELECT * FROM forum WHERE status='published' AND tags LIKE '%$Search%' ORDER BY id DESC LIMIT $page_1, $per_page ";
                        $check = Query($sql);
                        confirm($check);
                        ...
                        if(!fetch_data($result)){
                            ...
                            <script type="text/javascript">
                                alert('Error! No Result(s) For <?php echo $Search; ?>');
                            ...
                            <nav aria-label="Page navigation example">
                                <ul class="pagination">
                                    ...
                                    if($page > 1){
                                        ?>
                                        <li class="page-item">
                                            <a class="page-link" href="forum_search.php?search=<?php echo $Search; ?>&page=<?php echo $page-1; ?>" aria-label="Previous">
                                    ...
                                    {
                                        if($i == $page){
                                            echo "<li class='page-item active'><a class='page-link' href='forum_search.php?search=<?php echo $Search; ?>&page={$i}'>{$i}</a></li>";
                                        }else{
                                            echo "<li><a class='page-link' href='forum_search.php?search=<?php echo $Search; ?>&page={$i}'>{$i}</a></li>";
                                        }
                                    }
                                    ...
                                    if($i>$page_1){
                                        if($count){
                                            ?>
                                            <li class="page-item">
                                                <a class="page-link" href="forum_search.php?search=<?php echo $Search; ?>&page=<?php echo $page+1; ?>" aria-label="Next">
